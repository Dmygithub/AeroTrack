...
def generic_nms(
    ...
    if ious.is_cuda:
        if GENERIC_NMS_AVAILABLE:
            ...
            # nms_triton uses tl.cast which requires Triton 3.x (PyTorch 2.4+).
            # Use pure-PyTorch GPU implementation instead.
            return generic_nms_torch(ious, scores, iou_threshold)

    return generic_nms_cpu(ious, scores, iou_threshold)


def generic_nms_torch(
    ious: torch.Tensor, scores: torch.Tensor, iou_threshold=0.5
) -> torch.Tensor:
    """Vectorized GPU NMS using pure PyTorch (no Triton).

    Args:
        ious: Pairwise IoU matrix, shape (N, N).
        scores: Detection scores, shape (N,).
        iou_threshold: Suppression threshold.

    Returns:
        Indices of kept detections sorted by descending score.
    """
    n = scores.shape[0]
    if n == 0:
        return torch.zeros(0, dtype=torch.int64, device=scores.device)
    _, order = torch.sort(scores, descending=True, stable=True)
    iou_sorted = ious[order][:, order].contiguous()
    keep = torch.ones(n, dtype=torch.bool, device=ious.device)
    for i in range(n - 1):
        if keep[i]:
            keep[i + 1 :] &= iou_sorted[i, i + 1 :] <= iou_threshold
    return order[keep]
