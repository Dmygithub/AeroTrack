...
from aerotrack_core.models.sam3.model.sam3_tracker_base import concat_points, NO_OBJ_SCORE, Sam3TrackerBase
from aerotrack_core.models.sam3.model.sam3_tracker_utils import fill_holes_in_mask_scores
from aerotrack_core.models.sam3.model.utils.sam2_utils import load_video_frames
...
class Sam3TrackerPredictor(Sam3TrackerBase):
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(
            ...
            disable=should_disable_tqdm(tqdm_disable),
        ):
    ...
    def remove_object(self, inference_state, obj_id, strict=False, need_output=True):
        ...
        def _slice_state(output_dict, storage_key):
            for frame_idx, out in output_dict[storage_key].items():
                # maskmem_features / maskmem_pos_enc 在 run_mem_encoder=False 的帧
                # 合法为 None（如 propagate 的非关键帧），需要先判断再切片
                mf = out.get("maskmem_features")
                out["maskmem_features"] = (
                    mf[remain_old_obj_inds] if mf is not None else None
                )
                mp = out.get("maskmem_pos_enc")
                if mp is not None:
                    out["maskmem_pos_enc"] = [
                        x[remain_old_obj_inds] for x in mp
                    ]
                    # "maskmem_pos_enc" is the same across frames, only one copy needed
                ...
                if self.use_memory_selection:
                    iou = out.get("iou_score")
                    if iou is not None:
                        out["iou_score"] = iou[remain_old_obj_inds]
    ...
    def _apply_object_wise_non_overlapping_constraints(
        ...
        # Apply pixel-wise non-overlapping constraint based on mask scores.
        # NOTE: super()._apply_non_overlapping_constraints now operates in-place,
        # so pred_masks_single_score is modified in-place and the result is returned.
        pixel_level_non_overlapping_masks = super()._apply_non_overlapping_constraints(
            pred_masks_single_score
        )
        # Replace object scores with pixel scores. Note, that now only one object can claim
        # the overlapping region. Use masked_fill_ for true zero-copy in-place suppression;
        # fancy-indexing clamp still materialises a temporary tensor and can OOM.
        suppress = pixel_level_non_overlapping_masks <= 0
        if pred_masks.dtype == torch.bool:
            pred_masks.masked_fill_(suppress, False)
        else:
            pred_masks.masked_fill_(suppress, background_value)
        return pred_masks
