...
class Sam3VideoPredictor:
    ...
    def __init__(
        ...
        from aerotrack_core.models.sam3.model_builder import build_sam3_video_model
    ...
    def close_session(self, session_id):
        ...
        if session is None:
            ...
            # 显式释放 inference_state 中的大型 GPU tensor，
            # input_batch 持有整段视频所有帧的 CUDA tensor（约 12MB/帧），
            # 必须在 del 前主动解引用，否则 Python GC 可能延迟回收导致跨视频显存泄漏。
            ...
            try:
                ...
                if input_batch is not None:
                    ...
                    if isinstance(img_batch, list):
                        img_batch.clear()          # 正确清空 list 内元素引用
                    input_batch.img_batch = None
                inference_state["input_batch"] = None
