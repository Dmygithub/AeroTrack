...
from aerotrack_core.models.sam3 import perflib
from aerotrack_core.models.sam3.logger import get_logger
from aerotrack_core.models.sam3.model.act_ckpt_utils import clone_output_wrapper
from aerotrack_core.models.sam3.model.box_ops import box_xywh_to_cxcywh, box_xyxy_to_xywh
from aerotrack_core.models.sam3.model.data_misc import BatchedDatapoint, convert_my_tensors, FindStage
from aerotrack_core.models.sam3.model.geometry_encoders import Prompt
from aerotrack_core.models.sam3.model.io_utils import IMAGE_EXTS, load_resource_as_video_frames
from aerotrack_core.models.sam3.model.sam3_tracker_utils import fill_holes_in_mask_scores
from aerotrack_core.models.sam3.model.sam3_video_base import MaskletConfirmationStatus, Sam3VideoBase
from aerotrack_core.models.sam3.model.utils.misc import copy_data_to_device
from aerotrack_core.models.sam3.model.utils.progress import should_disable_tqdm
from aerotrack_core.models.sam3.perflib.compile import compile_wrapper, shape_logging_wrapper
from aerotrack_core.models.sam3.perflib.masks_ops import masks_to_boxes as perf_masks_to_boxes
...
class Sam3VideoInference(Sam3VideoBase):
    ...
    def _get_visual_prompt(self, inference_state, frame_idx, boxes_cxcywh, box_labels):
        ...
        if is_new_visual_prompt:
            if boxes_cxcywh.size(0) < 1:
                raise RuntimeError(
                    "visual prompts (box as an initial prompt) require at least one box, "
                    f"but got {boxes_cxcywh.shape=}"
                )
            if boxes_cxcywh.size(0) > 1:
                logging.warning(
                    "visual prompt expects a single box; using the first box as the visual prompt "
                    "and treating the remaining boxes as geometric prompts."
                )
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(
            ...
            disable=should_disable_tqdm(self.rank > 0),
        ):
    ...
    def _postprocess_output(
        ...
        if len(curr_obj_ids) == 0:
            ...
            keep = out_binary_masks.flatten(1).any(dim=1).cpu()  # remove masks with 0 areas
    ...
    def _warm_up_vg_propagation(self, inference_state, start_frame_idx=0):
        ...
        for _ in tqdm(range(num_iters), disable=should_disable_tqdm()):
...
class Sam3VideoInferenceWithInstanceInteractivity(Sam3VideoInference):
    ...
    def propagate_in_video(
        ...
        if propagation_type == "propagation_fetch":
            for frame_idx in tqdm(processing_order, disable=should_disable_tqdm()):
                if self.rank == 0:
