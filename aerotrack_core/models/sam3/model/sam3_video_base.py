...
from aerotrack_core.models.sam3.model.box_ops import fast_diag_box_iou
from aerotrack_core.models.sam3.model.data_misc import BatchedDatapoint
from aerotrack_core.models.sam3.model.sam3_tracker_utils import fill_holes_in_mask_scores, mask_to_box
from aerotrack_core.models.sam3.perflib.masks_ops import mask_iou
from aerotrack_core.models.sam3.train.masks_ops import rle_encode
...
class Sam3VideoBase(nn.Module):
    ...
    def _suppress_overlapping_based_on_recent_occlusion(
        ...
        if batch_size > 0:
            ...
            is_obj_occluded = ~(binary_tracker_low_res_masks_global.flatten(-2).any(dim=-1))
    ...
    def _associate_det_trk(
        ...
        if trk_masks.size(0) == 0:
            ...
            trk_is_nonempty = (trk_masks > 0).flatten(1).any(dim=1).cpu().numpy()
        ...
        trk_is_nonempty = trk_masks_binary.flatten(1).any(dim=1).cpu().numpy()
    ...
    def _process_hotstart(
        ...
        for obj_id in new_det_obj_ids:
            ...
            if obj_id in trk_keep_alive:
                # Defensive: obj_id shows up as "new detection" while still registered
                # in trk_keep_alive.  Occurs at high max_objects + low box_threshold
                # (person tracking) when detector and tracker disagree on identity.
                # Safe to reinitialize keep-alive; object continuity is preserved.
        ...
        for obj_id, frame_indices in unmatched_frame_inds.items():
            ...
            # Guard against obj_ids that were never registered in obj_first_frame_idx
            # (can happen due to hotstart timing edge cases)
            if obj_id not in obj_first_frame_idx:
                continue
        ...
        for _, matched_trk_obj_ids in det_to_matched_trk_obj_ids.items():
            ...
            # Filter out obj_ids not yet registered in obj_first_frame_idx
            # (can happen when a matched track was added before hotstart began tracking it)
            valid_matched = [x for x in matched_trk_obj_ids if x in obj_first_frame_idx]
            if len(valid_matched) < 2:
                continue
            ...
            first_appear_obj_id = (
                min(valid_matched, key=lambda x: obj_first_frame_idx[x])
                if not reverse
                else max(valid_matched, key=lambda x: obj_first_frame_idx[x])
            )
    ...
    def _tracker_update_memories(
        ...
        object_score_logits = torch.where(
            (high_res_masks > 0).flatten(-2).any(dim=-1), 10.0, -10.0
        )
