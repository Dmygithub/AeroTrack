...
class YOLOWorldHead(YOLOv8Head):
    ...
    def predict_by_feat(self,
        ...
        for (bboxes, scores, objectness,
            ...
            # mmcv nms only supports fp32/fp16; ensure fp32 to avoid bfloat16 crash
            if results.bboxes.dtype not in (torch.float32, torch.float16):
                results.bboxes = results.bboxes.float()
            if results.scores.dtype not in (torch.float32, torch.float16):
