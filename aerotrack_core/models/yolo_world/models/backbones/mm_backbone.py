...
class HuggingCLIPLanguageBackbone(BaseModule):
    def __init__(self,
        ...
        # Prefer project-local CLIP cache under checkpoints/clip-vit-base-patch32.
        # If该目录存在，则完全本地加载；否则按模型名自动从 HuggingFace 下载，
        # 并将权重缓存到该目录，方便之后离线复现。
        cwd = os.getcwd()
        local_dirs = [
            os.path.join(cwd, "checkpoints", "clip-vit-base-patch32"),
            os.path.join(cwd, "checkpoints", "Clip-vit-base-patch32"),
        ]
        local_dir = next((d for d in local_dirs if os.path.isdir(d)), None)
        ...
        if local_dir is not None:
            ...
            source = model_name
            local_only = False
            cache_dir = os.path.join(cwd, "checkpoints", "clip-vit-base-patch32")

        self.tokenizer = AutoTokenizer.from_pretrained(
            source, cache_dir=cache_dir, local_files_only=local_only
        )
